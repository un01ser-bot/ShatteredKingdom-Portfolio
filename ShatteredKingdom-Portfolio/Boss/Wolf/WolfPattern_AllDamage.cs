...
public class WolfPattern_AllDamage : MonoBehaviour, IBossPattern
{
    ...
    public void Tick(float dt)
    {
        ...
        if ((players == null || players.Length == 0) && target != null)
            players = new[] { target.gameObject };

        for (int i = 0; i < players.Length; i++)
        {
            Transform pt = players[i].transform;
            if (!IsInCircleWorld(pt, r)) continue;

            PlayerBase ph = players[i].GetComponent<PlayerBase>();
            if (ph != null)
            {
                ...
                Debug.Log($"전체데미지 히트, 데미지 : {finalDamage}");
            }
        }
